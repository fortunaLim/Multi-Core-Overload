# 1 "0_Src/0_AppSw/Tricore/System/DataStructure/Process_Queue_sh.c"
...
# 9 "0_Src/0_AppSw/Tricore/System/DataStructure/Process_Queue_sh.c" 2
...
# 10 "0_Src/0_AppSw/Tricore/System/DataStructure/Process_Queue_sh.c" 2
...
void enqueue_offset(QueueType* q, task_info task_data, int offset)
{
 ...
 else if((q->front+1+offset)% 32 == q->rear)
 {
 ...
 else
 {
  ...
  do
  {
   q->task_info[((q->rear)-idx)% 32] = q->task_info[((q->rear)-idx-1)% 32];
   idx++;
  }while(((q->front)+1+offset)% 32 != ((q->rear-idx))% 32);
  q->task_info[(q->front+1+offset)% 32] = task_data;
...
task_info dequeue(QueueType* q) {
 ...
 if (is_empty(q)==1) {
  ...
  memset(&(q->task_info[q->front]), 0, sizeof(task_info));
  return task_temp;
 }
}
...
task_info dequeue_back(QueueType* q) {
 ...
 if (is_empty(q)==1) {
  ...
  memset(&(q->task_info[q->rear]), 0, sizeof(task_info));
  q->rear = (q->rear - 1) % 32;
  return task_temp;
 }
}
...
task_info read_queue_front(QueueType* q) {
# 151 "0_Src/0_AppSw/Tricore/System/DataStructure/Process_Queue_sh.c"
 if (is_empty(q)) {

 }
 return q->task_info[(q->front + 1) % 32];
}

task_info read_queue_offset(QueueType* q, int offset) {
 task_info temp;
 memset(&temp, 0, sizeof(task_info));
 if (is_empty(q)) {

 }
 else {

  return q->task_info[(q->front + 1 + offset) % 32];
 }
...
# 311 "0_Src/0_AppSw/Tricore/System/DataStructure/Process_Queue_sh.c"
void Sort_By_EDF(int idx, int PQ_SIZE,QueueType *priority)
{
...
void Sort_By_EDF1(int idx,QueueType *q)
{
 ...
 be = q->task_info[(q->front+1)%32];

 for (i = 0; i < idx; i++) {
  for (j = 0; j < (idx - 1) - i; j++) {
   a = q->task_info[(q->front+1+j)%32];
   b = q->task_info[(q->front+2+j)%32];
   if (a.deadline > b.deadline)
   {
    task_temp = q->task_info[(q->front+1+j)%32];
    q->task_info[(q->front+1+j)%32] = q->task_info[(q->front+2+j)%32];
    b = q->task_info[(q->front+2+j)%32] = task_temp;
   }
  }
 }

 af = q->task_info[(q->front+1)%32];
...
# 403 "0_Src/0_AppSw/Tricore/System/DataStructure/Process_Queue_sh.c"
}
...
void increase_deadline(QueueType *q)
{
 ...
 {
  if(q->task_info[(q->front+1+i)%32].task_num != 0)
   q->task_info[(q->front+1+i)%32].deadline_ += 1;
 }
}
