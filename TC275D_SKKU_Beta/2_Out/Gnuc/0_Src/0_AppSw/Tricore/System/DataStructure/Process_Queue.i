# 1 "0_Src/0_AppSw/Tricore/System/DataStructure/Process_Queue.c"
...
# 1 "0_Src/0_AppSw/Tricore/System/DataStructure/Process_Queue.h" 1
# 11 "0_Src/0_AppSw/Tricore/System/DataStructure/Process_Queue.h"
# 1 "./0_Src/0_AppSw/Tricore/System/Service_Handler/Frame_Define.h" 1
# 11 "./0_Src/0_AppSw/Tricore/System/Service_Handler/Frame_Define.h"
...
# 12 "./0_Src/0_AppSw/Tricore/System/Service_Handler/Frame_Define.h" 2
...
# 12 "0_Src/0_AppSw/Tricore/System/DataStructure/Process_Queue.h" 2




typedef struct Node Queue_node_ptr;
typedef struct Head Queue_head_ptr;

void initial_Queue(int Queue_mode);
void RQ_enqueue(CAN_Network_Frame CANFrame);
Queue_node_ptr *RQ_dequeue(int ctrl);
void RQ_Sorting(Queue_node_ptr *new_node, Queue_node_ptr *com_node, int ctrl);
...
typedef struct Node
{
 ...
 unsigned int process_time;
 uint32 time_capture;
 Queue_node_ptr *prev_Link, *next_Link;
}Queue_node_ptr;
...
# 9 "0_Src/0_AppSw/Tricore/System/DataStructure/Process_Queue.c" 2
...
# 10 "0_Src/0_AppSw/Tricore/System/DataStructure/Process_Queue.c" 2
...
uint8 compare_mode = 0;
uint32 data_length = 0;

extern Queue_node_ptr old;
...
void RQ_Sorting(Queue_node_ptr *new_node, Queue_node_ptr *com_node, int ctrl)
{
 ...
 {
  case front_queue:
   com_node->prev_Link = new_node;
   new_node->next_Link = com_node;
   new_node->prev_Link = ((void *)0);
   head.node = new_node;
   break;
  case inner_queue:
   new_node->next_Link = com_node;
   com_node->prev_Link->next_Link = new_node;
   new_node->prev_Link = com_node->prev_Link;
   new_node->next_Link->prev_Link = new_node;
   break;
  case back_queue:
   com_node->next_Link = new_node;
   new_node->prev_Link = com_node;
   new_node->next_Link = ((void *)0);
   break;
 }
}
...
{
 Queue_node_ptr *temp;
 Queue_node_ptr *new = (Queue_node_ptr*)malloc(sizeof(Queue_node_ptr));

 memset(new, 0, sizeof(Queue_node_ptr));

 uint8 idx = 0, node_count = 0, sort_count = 0;

 new->priority = CANFrame.ID;
 new->DLC = CANFrame.DLC;
 new->process_time = CANFrame.process_time;

 for (idx = 0; idx < sizeof(CANFrame.Data); idx++)
  new->data[idx] = CANFrame.Data[idx];

 if(head.node == ((void *)0))
 {

  head.num = 1;
  new->prev_Link = ((void *)0);
  new->next_Link = ((void *)0);

  head.node = new;


 }
 else if (head.node != ((void *)0))
 {
  head.num++;
  ...
  switch(mode) {
   case Queue_FIFO:
    ...
    if(temp->next_Link == ((void *)0))
     RQ_Sorting(new, temp, back_queue);
    else
    {
     while(temp->next_Link != ((void *)0))
      temp = temp->next_Link;
   ...
   case Queue_EDF:
    ...
    if(temp->next_Link == ((void *)0))
    {
     if(temp->process_time > new->process_time)
      RQ_Sorting(new, temp, front_queue);
     else
      RQ_Sorting(new, temp, back_queue);
    }
    else
    {
     do {
      ...
      if(temp->process_time > new->process_time)
      {
       ...
       {
        sort_count++;
        RQ_Sorting(new, temp, inner_queue);
        break;
       }
      }
...
Queue_node_ptr *RQ_dequeue(int ctrl)
{
 ...
 {
  case front_queue:
   if(ptr->next_Link == ((void *)0))
    head.node = ((void *)0);
   else
   {
    head.node = ptr->next_Link;
    head.node->prev_Link = ptr->prev_Link;
   }
   break;
  case back_queue:
   if(ptr->next_Link == ((void *)0))
   ...
   {
    while(ptr->next_Link != ((void *)0))
     ptr = ptr->next_Link;
    ptr->prev_Link->next_Link = ((void *)0);
   }
   break;
 }
