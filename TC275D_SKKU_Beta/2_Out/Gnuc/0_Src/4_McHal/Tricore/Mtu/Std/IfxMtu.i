...
# 50 "0_Src/4_McHal/Tricore/Mtu/Std/IfxMtu.h" 2
...
static void IfxMtu_wait(uint32 towerDepth, uint8 numInstructions, IfxMtu_MbistSel mbistSel);
...
void IfxMtu_clearSram(IfxMtu_MbistSel mbistSel)
{
    ...
    mc->MCONTROL.U = mcontrolMask | (1 << (4)) | (1 << (0));
    mc->MCONTROL.U = mcontrolMask | (0 << (4)) | (1 << (4));


    IfxScuWdt_setSafetyEndinit(password);


    IfxMtu_wait(IfxMtu_sramTable[mbistSel].mbistDelay, 1, mbistSel);
    ((void)0);

    while (!mc->MSTATUS.B.DONE)
    {}
...
{
    Ifx_MC *mc = (Ifx_MC *)((0xF0061000u) + 0x100 * mbistSel);
    uint8 validFlags = (mc->ECCD.U >> (5)) & (0x1f);
...
void IfxMtu_readSramAddress(IfxMtu_MbistSel mbistSel, uint16 sramAddress)
{
    ...
    uint16 mcontrolMask = 0x4000;
    mc->MCONTROL.U = mcontrolMask | (1 << (3));
    mc->CONFIG0.U = (1 << (12)) | (1 << (0));
    mc->CONFIG1.U = 0;
    ...
    mc->MCONTROL.U = mcontrolMask | (1 << (3)) | (1 << (0));
    mc->MCONTROL.U = mcontrolMask | (1 << (3));


    IfxMtu_wait(256, 1, mbistSel);
    ((void)0);
...
uint8 IfxMtu_runCheckerBoardTest(IfxMtu_MbistSel mbistSel, uint8 rangeSel, uint8 rangeAddrUp, uint8 rangeAddrLow, uint16 *errorAddr, uint32 numberRedundancyLines)
{
    ...
    for (testStep = 0; testStep < 4; ++testStep)
    {
        ...
        IfxMtu_wait(IfxMtu_sramTable[mbistSel].mbistDelay, 4, mbistSel);
        ((void)0);
