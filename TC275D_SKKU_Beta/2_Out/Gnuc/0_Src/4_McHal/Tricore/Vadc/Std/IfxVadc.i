...
void IfxVadc_disablePostCalibration(Ifx_VADC *vadc, IfxVadc_GroupId group, unsigned char disable)
{
    if (group < (8))
    {
        IfxVadc_enableAccess(vadc, IfxVadc_Protection_globalConfig);

        uint32 mask = 1 << ((16) + group);
...
unsigned char IfxVadc_isPostCalibration(Ifx_VADC *vadc, IfxVadc_GroupId group)
{
    unsigned char pcEnabled;
...
void IfxVadc_setArbiterPriority(Ifx_VADC_G *vadcG, unsigned char slotEnable, IfxVadc_RequestSlotPriority prio, IfxVadc_RequestSlotStartMode mode, IfxVadc_RequestSource slot)
{
    if (slotEnable != 0)
    {
        vadcG->ARBPR.U |= slotEnable << ((24) + slot);
        vadcG->ARBPR.U &= ~((0x3) << (slot * 4u));
        vadcG->ARBPR.U |= (prio << (slot * 4u));

        if (mode != IfxVadc_RequestSlotStartMode_waitForStart)
        {
            vadcG->ARBPR.U |= 0x1u << ((3) + (slot * 4u));
        }
        else
        {
            vadcG->ARBPR.U &= ~(0x1u << ((3) + (slot * 4u)));
...
{
    unsigned char calibrationRunning;
