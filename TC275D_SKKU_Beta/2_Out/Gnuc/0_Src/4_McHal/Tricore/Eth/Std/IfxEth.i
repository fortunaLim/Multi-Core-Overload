...
void IfxEth_init(IfxEth *eth, const IfxEth_Config *config)
{
    ...
    (*(volatile Ifx_ETH_INTERRUPT_ENABLE *)0xF001F01Cu).U = 0x00010040;
...
void IfxEth_wakeupReceiver(IfxEth *eth)
{
    ...
    if (eth->status.U & (4U << (17)))
    {
        if (eth->status.B.RU)
        {
            (*(volatile Ifx_ETH_STATUS *)0xF001F014u).U = ((0x1) << (7));
        }
...
void IfxEth_wakeupTransmitter(IfxEth *eth)
{
    ...
    if (eth->status.U & 0x00600000)
    {
        if (eth->status.B.TU)
        {
            ...
            (*(volatile Ifx_ETH_STATUS *)0xF001F014u).U = ((0x1) << (2)) |
                           ((0x1) << (5));
        }
