...
IfxMultican_Status IfxMultican_Can_MsgObj_init(IfxMultican_Can_MsgObj *msgObj, const IfxMultican_Can_MsgObjConfig *config)
{
    ...
    unsigned char longFrame = (config->control.messageLen > IfxMultican_DataLengthCode_8) ? 1 : 0;


    unsigned char receiveFrame = (config->frame == IfxMultican_Frame_receive) ||
                                      (config->frame == IfxMultican_Frame_remoteAnswer) ||
                                      (config->frame == IfxMultican_Frame_remoteRequest);


    unsigned char transmitFrame = ((config->frame == IfxMultican_Frame_transmit) ||
                             (config->frame == IfxMultican_Frame_remoteAnswer) ||
                             (config->frame == IfxMultican_Frame_remoteRequest)) &&
                            (config->gatewayTransfers != 1);


    unsigned char gatewaySourceObj = (config->gatewayTransfers != 0) ? 1 : 0;


    unsigned char receiveFifo = 0, transmitFifo = 0;
    ...
    unsigned char gatewayFifo = (config->msgObjCount > 1) && (gatewaySourceObj);
    ...
    }
    {
        ...
        unsigned char receiveInterruptEnabled = (config->rxInterrupt.enabled) && (receiveFrame);
        IfxMultican_MsgObj_setReceiveInterrupt(hwObj, receiveInterruptEnabled);


        unsigned char transmitInterruptEnabled = (config->txInterrupt.enabled) && (transmitFrame);
        IfxMultican_MsgObj_setTransmitInterrupt(hwObj, transmitInterruptEnabled);


        unsigned char remoteMonitoringEnabled = (config->frame == IfxMultican_Frame_remoteAnswer);
...
unsigned char IfxMultican_Can_MsgObj_isRxPending(IfxMultican_Can_MsgObj *msgObj)
{
...
unsigned char IfxMultican_Can_MsgObj_isTxPending(IfxMultican_Can_MsgObj *msgObj)
{
    if (msgObj->msgObjCount > 1)
    {
        Ifx_CAN_MO *hwObj = IfxMultican_MsgObj_getPointer(msgObj->node->mcan, msgObj->fifoPointer);
...
# 1005 "0_Src/4_McHal/Tricore/Multican/Can/IfxMultican_Can.c"
    for (i = 0; i < 8; i++)
    {
