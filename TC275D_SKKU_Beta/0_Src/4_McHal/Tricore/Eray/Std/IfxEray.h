...
typedef enum
{
    ...
    IfxEray_WakeupChannel_none=2 /**< \brief send wakeup channel pattern on none */
...
IFX_EXTERN void IfxEray_readData_Byte(Ifx_ERAY *eray, uint8 *data, uint8 payloadLength);
 ...
 * \return None
 */
IFX_EXTERN void IfxEray_setSlot(Ifx_ERAY *eray, IfxEray_Header *header, uint32 *data, IfxEray_SlotConfig *slotConfig);
 ...
 * \return None
 */
IFX_EXTERN void IfxEray_writeData(Ifx_ERAY *eray, uint32 *data, uint8 payloadLength);
...
IFX_INLINE void IfxEray_setAutoDelayBuffers(Ifx_ERAY *eray)
{
    eray->CUST1.B.IEN = 1; // auto delay scheme for Input Buffer enable
    eray->CUST1.B.OEN = 1; // auto delay scheme for Output Buffer enable
