...
IfxVadc_Status IfxVadc_Adc_initModule(IfxVadc_Adc *vadc, const IfxVadc_Adc_Config *config)
{
    ...
    /* for SH-ADC: select converter clock */
    IFX_ASSERT(IFX_VERBOSE_LEVEL_WARNING, config->converterClock == IfxScuCcu_AdcClockSelection_backup); // this is not a perfect clock selection
...
{
    static uint8  init_done[IFXVADC_NUM_ADC_CAL_GROUPS]        = {'\0'};
    static uint16 adc_gain_calib[IFXVADC_NUM_ADC_CAL_GROUPS]   = {'\0'};
    static sint8  adc_offset_calib[IFXVADC_NUM_ADC_CAL_GROUPS] = {'\0'};
    uint32        AdcQMR_save;
    // will hold the result from a conversion
    Ifx_VADC_RES  result;
    // defines the count how many times the calibration is executed
    uint32        count;
    // saved content of GxCHCTR0 and 1
    uint32        saved_ch0, saved_ch1;
    // pointer to VADC structure
    Ifx_VADC     *pIfx_VADC;
    // set start address of VADC pointer
    pIfx_VADC = &MODULE_VADC;

    uint32        groupsPerShs = (IFXVADC_NUM_ADC_CAL_GROUPS / IFXVADC_NUM_ADC_SHS);

    // if this is first call we need to initialize and make startup calibration
    if (init_done[groupId] == 0)
    {
        // start with center offset value
        adc_offset_calib[groupId] = 0;
        // start with max. gain value
        adc_gain_calib[groupId]   = 0x3FFF;
        // 124 cycles are performed for startup calibration to approximate valid value
        count                     = 124;
    }
    else
    {
        // if it is not the startup calibration, make only one conversion
        count = 1;
    }

    AdcQMR_save = pIfx_VADC->G[groupId].QMR0.U;
    // save the content of GxCHCTR0
    saved_ch0   = pIfx_VADC->G[groupId].CHCTR[0].U;

    // setup GxCH0 to use local result register 0
    pIfx_VADC->G[groupId].CHCTR[0].U = 0;

    // save the content of GxCHCTR1
    saved_ch1 = pIfx_VADC->G[groupId].CHCTR[1].U;

    // setup GxCH1 to use local result register 0
    pIfx_VADC->G[groupId].CHCTR[1].U = 0;

    // if this is the startup calibration then make this more than 1 time
    while (count--)
    {
        //********switch CH0 to VAGND and CH1 to VAREF**************************
        pIfx_VADC->G[groupId].ALIAS.U = 0x1F1E;
        // insert a CH0 request in the queue
        pIfx_VADC->G[groupId].QINR0.U = (1 << 7) | (0x1f & 0);
        // trigger the request in the queue and start the conversion
        pIfx_VADC->G[groupId].QMR0.U  = (1 << 16) | (1 << 9) | 0x1;

        // discard a cancelled conversion, generate a trigger by software,
        // conversion request always issued
        ...
        // ignore the result from this conversion
        //********offset calibration********************************************
        // insert a CH0 request in the queue
        pIfx_VADC->G[groupId].QINR0.U = (1 << 7) | (0x1f & 0);
        // trigger the request in the queue and start the conversion
        ...
        // increment or decrement offset correction value dependent on the result
        // max. value 63, min. value -60
        ...
        //MEM (0xF00250E0+ 4*(groupId%groupsPerShs)+ 0x200*(groupId/groupsPerShs)) =	(adc_offset_calib[groupId] & 0x7F) | (1<<15) | (1<<31);
        //********gain calibration**********************************************
        // insert a CH1 request in the queue
        ...
        // increment or decrement gain value dependent on the result max. value
        // 0x3FFF, min. value 0
        if ((result.B.RESULT < 0xFFF) && (adc_gain_calib[groupId] > 0))
        {
            // in case of startup calibration subtract more than only 1
            // to speedup approximation
            if (init_done[groupId] == 0)
            {
        ...
        //********switch CH0 and CH1 to normal in*******************************
